...
public class Coleccion {
    public static void main(String[] args) {
        ...
        //  Long canCur= cursos.stream().filter(n-> n.getDuracion() >5 ).mapToInt(n->n.getVideos()).count();
        ...
        //   Long canCur2= cursos.stream().filter(n-> n.getDuracion() <2 ).mapToInt(n->n.getVideos()).count();
    ...
    //  cursos.stream().sorted(Comparator.comparing(Curso::getDuracion).reversed()).limit(1).forEach(nomCur -> System.out.println(nomCur.getTitulo()));
        Curso maximo = cursos.stream().max(Comparator.comparing(m ->((Curso) m).getDuracion())).get();
        System.out.println("El curso con mayor duración es: "+maximo.getTitulo());
